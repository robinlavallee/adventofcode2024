...
int solve(std::vector<std::vector<int>> maze) {
    ...
    while (!toProcess.empty()) {
        ...
        for (auto it = pointCandidate->second.begin() ; it != pointCandidate->second.end(); ++it) {
            ...
            for (int direction : directions) {
                ...
                auto visitedIt = visitedEntry.find(nextDirectionCount.dirCount);
                if (visitedIt == visitedEntry.end()) {
                    visitedEntry[nextDirectionCount.dirCount] = nextDirectionCount.cost;
                    toProcess[nextDirectionCount.point][nextDirectionCount.dirCount] = nextDirectionCount.cost;
                } else {
                    // There cannot be more than 3 entries in the map
                    bool ignore=false;
                    for (auto it = visitedEntry.begin(); it != visitedEntry.end();) {
                        auto& visitedElem = *it;

                        // Replace worse entries (new entry is better on cost and direction count)
                        if (visitedElem.first.count > nextDirectionCount.dirCount.count && visitedElem.second > nextDirectionCount.cost) {
                            // Remove from toProcess as well
                            auto toProcessIt = toProcess[nextDirectionCount.point].find(visitedElem.first);
                        ...
                        if (visitedElem.second == nextDirectionCount.cost && visitedElem.first.count > nextDirectionCount.dirCount.count) {
                            ignore = true;
                            ++it;
                            continue;
                        }
    ...
    for (auto& entry : visited[maze.size() - 1][maze[0].size() - 1]) {
        if (entry.second < bestCost) {
            bestCost = entry.second;
        }
    }
